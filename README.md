...
|-----------|-------|
| 歪度（Skewness） | 0.2 |
| 非直交性（Non-orthogonality） | 10.0 |
| アスペクト比（Aspect Ratio） | 5.0 |
| サイズジャンプ（Size Jump） | 1.5 |

重みは1.0（良好な品質）からW_PDE_MAX=20.0（低品質）の範囲で変動し、モデルが困難な領域において物理的に整合性のある予測により多くの注意を払うことを保証します。
